<template>
  <PageNoScroll class="batchnofixedCostSettingModel">
    <div v-if="isSetting == 1">
          ...
          <el-button
            ...
            @click="
              handleDialog('historyconfig', '历史配置', {
                jump: '/costsetting/batchnofixedCostSetting',
                ...tableData,
              })
            "
            >历史配置</el-button
      ...
      <div style="margin-bottom: 10px" class="flex_dom flex_item_mid">
        ...
        <div
          v-for="(item, index) in pageData.month_data"
          :key="index"
  ...
  methods: {
          ...
          productCate({
            ...
            if (res.status == 200) {
              ...
              if (this.tableData.uid) {
                setTimeout(() => {
                  this.getList1(this.tableData.uid);
                }, 0);
              }
            }
          });
    ...
    getList() {
      getCostunfixd({
        ...
        // this.form.transportMoney = this.pageData.transportMoney; // 运输费
        // this.form.loaddingMoney = this.pageData.loaddingMoney; // 装卸费
    ...
    handleChecked(val) {
      for (let i = 0; i < this.pageData.month_data.length; i++) {
        const el = this.pageData.month_data[i];
        if (el.is_set) {
    ...
    addtransactionfee(val) {
      ...
      if (val == 2) {
        ...
        this.isSetting = val;
        // this.infoFormData = {
        //   isSellbuy: this.isTypeSales, // 计费方式
        //   sellCost: '1', // 交易费类型
        //   sellCostMoney: 0 // 交易费用
        // }
        this.infoFormData.sellCost = "1"; // 交易费类型
        this.infoFormData.sellCostMoney = ""; //0; // 交易费用
