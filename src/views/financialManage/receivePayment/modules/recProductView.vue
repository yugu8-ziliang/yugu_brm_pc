<template>
  <!--商品配送  -->
  <div class="recProductView-model">
    <div class="recProductView-search">
      <div>
        <el-input
          style="width: 300px"
    ...
    <div class="recProductView-amount">
      ...
      <span
        style="
          ...
          margin-left: 10px;
        "
        >未收配送款：<span style="color: rgba(1, 6, 33, 0.9); font-size: 20px">
          {{ tableData.uncollectedOrder | intervalFormat(false) }}</span
        ></span
      >
    </div>

    <div class="recProductView-table">
      <u-table
        ref="plTable"
        class="utable"
...
export default {
  ...
  methods: {
    getList(arg) {
      ...
      let param = {
        orderType:5, // 订单类型，1 商品销售 2 筐子售出 3 退货退款 4 退筐退款 5 商品配送 6 记账 必传
...
.recProductView-model {
  .recProductView-search {
    padding-bottom: 15px;
    border-bottom: 1px solid #e9e9e9;
  }
  .recProductView-amount {
    margin-top: 15px;
