<template>
  <!-- 退货退款 -->
  <div
    class="paidRefundsViewModel"
    :style="{ '--screenHeight': $store.getters.screenHeight - 250 + 'px' }"
  >
    <div class="flex_dom">
      <div class="paidRefundsView-left">
        <el-scrollbar>
          <ul class="paidRefundsView-left-ul">
            <li
              v-for="(item, index) in tableDataList"
              :key="index"
              class="paidRefundsView-left-li flex_dom flex_item_mid flex_item_between"
              :style="{
      ...
      <div class="paidRefundsView-right">
        <div class="paidRefundsView-amount">
...
export default {
  ...
  methods: {
    getList(arg) {
      ...
      let param = {
        unpaidType: 3, // 未付类型    1-商品采购 2-筐子购入 3-退货退款 4-退筐退款 5-配送退货 6-配送退筐 7-记账
        pagesize: 0, // 当前第几页
        pagenum: 0, // 一页显示多少条
      };
...
.paidRefundsViewModel {
  ...
  .paidRefundsView-left {
    width: 300px;

    .paidRefundsView-left-ul {
      margin-right: 7px;
      .paidRefundsView-left-li {
  ...
  .paidRefundsView-right {
    width: calc(100% - 300px);
    .paidRefundsView-amount {
      margin-bottom: 10px;
