<template>
  <!--退货退款  -->
  <div class="refundsView-model">
    <div class="refundsView-search">
      <div>
        <el-input
          style="width: 270px"
    ...
    <div class="refundsView-amount">
      ...
      <span
        style="
          ...
          margin-left: 10px;
        "
        >待收退款：<span style="color: rgba(1, 6, 33, 0.9); font-size: 20px">
          {{ tableData.collectionAmount | intervalFormat(false) }}</span
        ></span
      >
    </div>

    <div class="refundsView-table">
      <u-table
        ref="plTable"
        class="utable"
...
export default {
  ...
  methods: {
    getList(arg) {
      ...
      let param = {
        orderType: 2, // 订单类型，1：销售款，2：退货退款，3：筐子售出，4：退筐退筐，5：转账
      ...
      userCollectionList(param).then((res) => {
        if (res.status == 200) {
          ...
          //  [
          //   {
          //     customerId: "9792D9511B4E143932FA40C58FF6D0AB",
          //     orderNo: 4033,
          //     timestamp: 1645507299,
          //     transferType: "预付款",
          //     orderType: 1,
          //     collectionAmount: 35552.0,
          //     relationOrders: 3,
          //     customerName: "错过的你",
          //     customerPhone: "15333333333",
          //     companyName: "测试1号",
          //     customerType: 1,
          //   },
          // ];
    ...
    handleJumpDetail: function (form, record) {
      this.$router.push({
        path: form,
        query: {
          data: Base64.encode(JSON.stringify({ typeIs: 1, ...record })),
        }, //加密
      });
