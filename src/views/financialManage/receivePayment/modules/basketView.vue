<template>
  <!--筐子售出  -->
  <div class="basketView-model">
    <div class="basketView-search">
      <div>
        <el-input
          style="width: 270px"
    ...
    <div class="basketView-amount">
      ...
      <span
        style="
          ...
          margin-left: 10px;
        "
        >待收筐子款：<span style="color: rgba(1, 6, 33, 0.9); font-size: 20px">
          {{ tableData.collectionAmount | intervalFormat(false) }}</span
        ></span
      >
    </div>

    <div class="basketView-table">
      <u-table
        ref="plTable"
        class="utable"
...
export default {
  ...
  methods: {
    getList(arg) {
      ...
      let param = {
        orderType: 3, // 订单类型，1：销售款，2：退货退款，3：筐子售出，4：退筐退筐，5：转账
    ...
    handleJumpDetail: function (form, record) {
      this.$router.push({
        path: form,
        query: {
          data: Base64.encode(JSON.stringify({ typeIs: 0, ...record })),
        }, //加密
      });
