<template>
  <!--转账  -->
  <div class="transferView-model">
    <div class="transferView-search">
      <div>
        <el-input
          style="width: 270px"
    ...
    <div class="transferView-amount">
      ...
      <span
        style="
          ...
          margin-left: 10px;
        "
        >未收转账：<span style="color: rgba(1, 6, 33, 0.9); font-size: 20px">
          {{ tableData.collectionAmount | intervalFormat(false) }}</span
        ></span
      >
    </div>

    <div class="transferView-table">
      <u-table
        ref="plTable"
        class="utable"
...
export default {
  ...
  methods: {
    getList(arg) {
      ...
      let param = {
        orderType: 5, // 订单类型，1：销售款，2：退货退款，3：筐子售出，4：退筐退筐，5：转账
      ...
      userCollectionList(param).then((res) => {
        if (res.status == 200) {
          ...
          // [
          //   {
          //     customerId: "9792D9511B4E143932FA40C58FF6D0AB", //# 客户id
          //     orderNo: 4033, //# 查询订单类型为1，2，3，4时时收款单主键id，为5时是转账单号
          //     timestamp: 1645507299, //# 时间
          //     transferType: "预付款", //# 转账类型，查询订单类型为5是有该字段
          //     orderType: 1, //# 订单类型，1：转账，2：记账，查询订单类型为5是有该字段
          //     collectionAmount: 35552.0, //# 待收金额
          //     relationOrders: 3, //# 关联订单个数
          //     customerName: "错过的你", //# 客户姓名
          //     customerPhone: "15333333333", //# 手机号
          //     companyName: "测试1号", //# 公司名称
          //     customerType: 1, //# 用户类型，1：经营户，2：采购户，3：供货商
          //   },
          // ];

          // res.data[0].modelList;
    ...
    handleJumpDetail: function (form, record) {
      ...
      // this.$router.push({
      //   path: form,
      //   query: { data: Base64.encode(JSON.stringify({typeIs:1,...record})) }, //加密
      // });
