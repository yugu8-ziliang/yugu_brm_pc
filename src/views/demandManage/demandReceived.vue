<template>
  <Container footer>
          ...
          <el-row style="margin-bottom: 15px" v-if="isshowtypeitem.all">
            ...
            <el-col :span="22" class="checked_type_styl">
              <el-tag
                ...
                v-for="(item, index) in validCheckedTypes"
                :key="'tag' + index"
                closable
                type="info"
              >
    ...
    <template v-if="activeName === '2'" #footer
    ...
    <template v-if="activeName == '1'" #footer>
      <el-button
        type="text"
        @click="isShowChecked"
        :style="
          checkedGoods.length > 0 && checkedGoods.length <= 20
            ...
            : 'color:red;'
        "
        v-if="checkedGoods.length > 0"
      >
        查看已选({{ checkedGoods.length + "/20" }})</el-button
      >
      <el-button
        type="primary"
        size="mini"
        :disabled="
          checkedGoods.length > 0 && checkedGoods.length <= 20 ? false : true
        "
        :class="{
          unActivate:
            checkedGoods.length > 0 && checkedGoods.length <= 20 ? false : true,
...
export default {
  ...
  computed: {
    validCheckedTypes() {
      return this.checkedType.filter(({ name }) => name);
    },
  },
  data() {
    return {
