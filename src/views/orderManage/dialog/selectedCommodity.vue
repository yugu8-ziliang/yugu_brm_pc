  ...
  methods: {
    ...
    handleRemove(row) {
      this.$confirm("确认移除此商品?", "提示", {
        ...
        .then(() => {
          this.tableDataListDelete.push(row.row);
    ...
    close(val) {
      ...
      this.model = {};
      this.$emit("selectedCommodityChange", {
        tableDataList: JSON.parse(JSON.stringify(this.tableDataList)),
        tableDataListDelete:this.tableDataListDelete,
      });
      this.tableDataList = [];
      this.tableDataListDelete = [];
    },
  },
};
</script>
