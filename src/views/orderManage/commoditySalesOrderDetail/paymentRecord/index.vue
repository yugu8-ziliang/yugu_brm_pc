                ...
                <el-col :span="21">
                  <div>
                    <span style="color: rgba(1, 6, 33, 0.75); font-size: 14px"
                      >线下付款：</span
                    ><span style="color: rgba(1, 6, 33, 0.75); font-size: 14px"
                      >{{ payInfoLoc(item.payInfo) }}元</span
                    >
                  </div>
                </el-col>
              </el-row>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      ...
      let param = {
        orderType: this.formDatas.ordertype == "1" ? 2 : 4, // 订单类型 int 1：筐子订单 2：商品订单 3：退货退款单 4：添加商品，添加入库 5:厂商自购筐子 6:退筐退款 7:筐子报废 9:配送退货 10:配送 11:配送退筐
