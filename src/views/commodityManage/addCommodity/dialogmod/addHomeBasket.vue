...
<script>
import { addOwnBasket } from "@/request/api/commodityManage/commodityList";
...
export default {
  ...
  methods: {
    ...
    getAddOwnBasket() {
      let param = {
        categoryname: this.model.productid, // 分类id
    ...
    handleAdd() {
      ...
      for (let index = 0; index < this.baskList.length; index++) {
        ...
        if (el.check) {
          ...
          if (dataP.length > 0) {
            padamList.push({ ...el, ismyseller: 1, basketinfo: dataP });
          }
        }
      }
      ...
      this.$emit("addHomeBasket", padamList);
      this.close();
    },
    // 清空数据
    clearTree() {},
