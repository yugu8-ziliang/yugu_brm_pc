  ...
  <Page v-loading="loading">
    ...
    <el-row v-if="tableData.length > 0" class="layout_styl_1">
      <el-col :span="23">
        <el-table :data="tableData" height="300px" style="border:1px solid #E9E9E9;">
          ...
          <el-table-column label="腾空前库存" prop="">
            <template slot-scope="scope">
              <span v-html="getStock(goodMsg.subUnit,goodMsg.subUnit2,goodMsg.subUnit3,goodMsg.pieceUnit,goodMsg.subUnit2Weight,goodMsg.subUnit3Weight,scope.row.batchnumber,scope.row.basketnumber,scope.row.piecenumber)"></span>
            </template>
          </el-table-column>
          <el-table-column label="腾空后库存" prop="soarAfter">
            <template slot-scope="scope">
              <span v-html="scope.row.soarAfter"></span>
            </template>
          </el-table-column>
          <el-table-column label="腾空装载物" prop="carrySoar"></el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              <el-button type="text" @click="tableDataEdit(scope.row,scope.row.batchid)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="table_count_styl">
          ...
          <span style="margin-right:20px;">腾空装载物:{{ countCarrySoar }}{{goodMsg.pieceUnit}}</span>
          <span style="margin-right:20px;">腾空筐子:{{ countSoar }}个</span>
        </div>
      </el-col>
    </el-row>
    <el-divider></el-divider>
    <h4 style="margin-top:15px;">腾空原因</h4>
    <el-input
      style="width:30%"
      class="layout_styl_1"
      type="textarea"
    ...
    >
      ...
      <el-row class="layout_styl_1 font_styl_1">
        ...
        <el-col :span="6">入库类型:
          <el-button @click="toPageType(editBatchMsg.batchsourcename)" type="text" size="mini" style="margin-top:-3px;">{{editBatchMsg.batchsourcename}}</el-button>
        </el-col>
        <el-col :span="6">当前库存:
      ...
      <el-form :model="form" :rules="rules" ref="form" label-position="left">
        <h4 v-if="editBatchMsg.piecenumber != 0">装载物信息</h4>
        <el-row class="font_styl_1" v-if="editBatchMsg.piecenumber != 0" style="border-bottom:1px solid #DCDFE6">
          <el-col :span="6">
            <el-form-item :label="goodMsg.pieceUnit"></el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="'载货:'+editBatchMsg.piecenumber+'个'"></el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="腾空:" prop="carrySoar" label-width="20%">
              <el-input v-model.number="form.carrySoar" size="mini" style="width:70%;margin-top:3px;">
        ...
        <div v-for="(item,index) in form.basketArr" :key="'name'+index" style="margin-top:15px;">
          <h5 style="margin-bottom:15px;">【{{item.basketType}}】{{ item.basketName }}</h5>
          <el-row v-for="(coloritem,colorindex) in item.info" :key="'color'+colorindex">
            ...
            <el-col :span="6">
              <el-form-item label="腾空:" required label-width="20%" :prop="'basketArr.'+ index + '.info.'+ colorindex " :rules="rules.soar">
                <el-input v-model.number="coloritem.soar" style="width:70%;margin-top:3px;" size="mini">
                  <template slot="append">个</template>
                </el-input>
              </el-form-item>
            </el-col>
        ...
        <el-divider></el-divider>
        <p style="text-align:right;height: 22px;font-size: 16px;font-weight: 500;color: rgba(1,6,33,0.9000)">共腾空{{ sumSoar }}个筐子</p>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
import { getBatch,getCarryBasket,getSoar} from '@/request/api/theOverflow/index'
export default {
  computed:{
    countSoar(){
      let res = 0;
  ...
  data() {
    var isExceed1 = (rule, value, callback) => {
      if(value > this.editBatchMsg.piecenumber){
        return callback(new Error('腾空数量不能大于当前载货数量！'));
      }else{
        callback();
      }
    };
    var isExceed2 = (rule, {nums,soar}, callback) => {
      if(nums < soar){
        return callback(new Error('腾空数量不能大于载货数量！'))
      }else{
        callback();
      }
    };
    return{
      ...
      rules:{
        carrySoar:[{required: true, message: '请填写腾空数量', trigger: 'blur' },{ validator:isExceed1, trigger: 'blur'}],
          ...
          if(valid){
            for(let i=0;i<this.tableData.length;i++){
              if(this.tableData[i].batchid == this.editBatchMsg.batchid){
                ...
                this.tableData[i].basketArr = JSON.parse(JSON.stringify(this.form.basketArr))
                this.tableData[i].carrySoar = this.form.carrySoar
                this.tableData[i].soarAfter = this.getStock(this.goodMsg.subUnit,this.goodMsg.subUnit2,this.goodMsg.subUnit3,
                                              this.goodMsg.pieceUnit,this.goodMsg.subUnit2Weight,this.goodMsg.subUnit3Weight,
                                              this.editBatchMsg.batchnumber,(this.editBatchMsg.basketnumber-this.sumSoar),(this.editBatchMsg.piecenumber-this.form.carrySoar))
              }
            }
          ...
          for(let i=0;i<res.length;i++){
            if(obj.indexOf(res[i].basketId) === -1){
              arr.push({
                ...
                info:[{basketColor:res[i].basketColor,nums:res[i].nums,soar:0}]
              })
    ...
    submit(type){
      ...
      for(let i=0;i<this.tableData.length;i++){
        if(!this.tableData[i].soarAfter){
          this.$error('编号'+ (i+1) + '的批次没有被编辑，请编辑后再做提交！');
          issubmit = false;
        }
      }
      if(issubmit){
        let res = {
          ...
          basketTotalCount:this.countSoar,
          basketTypeCount:0,//########!!!!
          pieceUnit:this.goodMsg.pieceUnit,
          pieceUnitCount:this.countCarrySoar,
          productId:this.id,
          emptyData:[]
        }
        //统计筐子种类
        ...
        let zhonglei = ([...new Set(basketIdArr)])
        res.basketTypeCount = zhonglei.length;
        //批次筐子信息
        let obj = []
        for(let i=0;i<this.tableData.length;i++){
          obj.push({
            productBatchId:this.tableData[i].batchid,
            emptyPieceNumber:this.tableData[i].carrySoar,
            productIdBatchName:this.toDate(this.tableData[i].timestamp),
            productIdBatchType:this.tableData[i].batchsourcename,
            productIdBatchSource:this.tableData[i].custmorName,
            basketList:[]
          })
        }
          ...
          for(let j=0;j<this.tableData.length;j++){
            if(obj[i].productBatchId == this.tableData[j].batchid){
              for(let k=0;k<this.tableData[j].basketArr.length;k++){
                for(let o=0;o<this.tableData[j].basketArr[k].info.length;o++){
                  obj[i].basketList.push({
                    basketId:this.tableData[j].basketArr[k].basketId,
                    productBindId:0,
                    emptyNumber:this.tableData[j].basketArr[k].info[o].soar,
                    basketColor:this.tableData[j].basketArr[k].info[o].basketColor
                  })
                }
              }
            }
        ...
        getSoar(res).then(resp=>{
          ...
          if(resp.status == 200){
            ...
            if(type == 2){
              ...
              this.$router.push({path:'emptyDetails',query:{id:resp.data[0]}})
            }
            this.$closePage();
          }
        })
