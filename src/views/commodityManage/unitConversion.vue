<template>
  <Page v-loading="loading">
    <div v-show="isShowEditGood == false">
      <h4>商品信息</h4>
      ...
      <el-row v-if="tableData.length > 0" class="layout_styl_1">
        <el-col :span="23">
          <el-table :data="tableData" height="300px" style="border:1px solid #E9E9E9;">
            ...
            <el-table-column label="转换数量" prop="transitionNums"></el-table-column>
            <el-table-column label="转换成本" prop="transitionCost"></el-table-column>
          ...
          <div class="table_count_styl">
            ...
            <span style="margin-right:20px;">转换总量:{{ getZongliang }}{{ goodMsg.subUnit }}</span>
            <span style="margin-right:20px;">转换成本总额:{{ getChengben }}元</span>
      ...
      <el-row class="layout_styl_1 font_styl_2" v-if="tableData.length > 0">
        ...
        <el-col :span="6">入库成本单价：{{ rukuchengbendanjia }}元/{{editGoodsMsg.subUnit}}</el-col>
      </el-row>
      <h4 style="margin-top:15px;">转换原因</h4>
      <el-input
        style="width:30%"
        class="layout_styl_1"
        type="textarea"
    ...
    <div v-show="isShowEditGood">
      ...
      <el-form :model="editGoodsMsg" :rules="rules" ref="editGoodsform" label-position="left" style="margin-top:15px;">
        <el-row>
          <el-col :span="8">
            <el-form-item prop="subUnit" label="计算单位">
              <br>
              <el-input v-model="editGoodsMsg.subUnitFocus" size="mini" style="width:50%" @focus="subunitFocus"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row>
          ...
          <el-col :span="8" v-if="editGoodsMsg.nums1 == '斤' || editGoodsMsg.nums1 == '公斤' || editGoodsMsg.nums1 == 'g'">
        ...
        <el-row>
          ...
          <el-col :span="4">
            <el-form-item label="入库成本单价">
              <br>
              {{ getChengbenDanjia + '元' }}{{ editGoodsMsg.subUnit == '' ? '' : '/'+editGoodsMsg.subUnit}} 
            </el-form-item>
          </el-col>
          <el-col :span="4">
        ...
        <div v-if="editGoodsMsg.ownbasket.length > 0">
          ...
          <div>
            <el-row v-if="editGoodsMsg.isQp == '1'" style="margin-top:15px">
              <el-col :span="24">商品毛重:
                {{(editGoodsMsg.nums1 + getsumweight)+editGoodsMsg.subUnit }}
                {{goodMsg.subUnit2 != '' ? '(':'' }}
                {{goodMsg.subUnit3Weight==0 ? '' :goodMsg.subUnit3Weight +goodMsg.subUnit3}}
                {{goodMsg.subUnit3 !='' ? '*' : '' }}
                {{goodMsg.subUnit2Weight==0 ? '' : goodMsg.subUnit2Weight +goodMsg.subUnit2}}
              ...
              <el-col :span="24">商品净重:{{ editGoodsMsg.nums1 }}{{editGoodsMsg.subUnit}}</el-col>
            </el-row>
            <el-row v-if="editGoodsMsg.isQp != '1'" style="margin-top:15px">
              <el-col :span="24">商品数量:
                {{editGoodsMsg.nums1+editGoodsMsg.subUnit }}
                {{goodMsg.subUnit2 != '' ? '(':'' }}
                {{goodMsg.subUnit3Weight==0 ? '' :goodMsg.subUnit3Weight +goodMsg.subUnit3}}
                {{goodMsg.subUnit3 !='' ? '*' : '' }}
                {{goodMsg.subUnit2Weight==0 ? '' : goodMsg.subUnit2Weight +goodMsg.subUnit2}}
    ...
    >
      ...
      <el-form :model="form" ref="form" :rules="rules" inline  label-position="left" style="margin-top:15px;">
        <el-form-item prop="transitionNums" label="转换数量" >
          <el-input size="mini" v-model.number="form.transitionNums" style="margin-top:5px;">
            <template slot="append">{{ goodMsg.subUnit }}</template>
          </el-input>
        </el-form-item>
        ...
        <el-form-item prop="transitionPiece" label="转换件数" v-if="goodMsg.subUnit !='斤' && goodMsg.subUnit !='公斤' && goodMsg.subUnit !='g'">
          <el-input size="mini" v-model.number="form.transitionPiece" style="margin-top:5px;">
            <template slot="append">{{ goodMsg.pieceUnit == '筐(一次性)' ? '筐' : goodMsg.pieceUnit }}</template>
          </el-input>
        </el-form-item>
        <span>转换成本
          <el-tooltip class="item" effect="dark" content="转换成本包含报溢商品批次报溢商品批次成本为0" placement="top">
        ...
        <el-row v-for="(item,index) in batchweighData" :key="'aa'+index" style="margin-bottom:10px;">
          ...
          <el-col :span="6">
            <el-input v-model.number="item.num1" size="mini" style="width:70%">
              <template slot="append">{{  judgebatchweig == '批次' ? goodMsg.subUnit : editGoodsMsg.subUnit }}</template>
            </el-input>
          </el-col>
          <el-col :span="6" v-if="
          (judgebatchweig == '批次' && (goodMsg.subUnit =='斤' || goodMsg.subUnit =='公斤' || goodMsg.subUnit =='g'))||
          (judgebatchweig == '商品') && (editGoodsMsg.subUnit =='斤' || editGoodsMsg.subUnit =='公斤' || editGoodsMsg.subUnit =='g')">
...
import { getBatch,getCarryBasket,getOwnBasket,getunitConversion } from '@/request/api/theOverflow/index'
export default {
  name:'unitConversion',
  data() {
    ...
    var isFit = (rule, value, callback) => {
      if(value == this.goodMsg.subUnit){
        return callback(new Error('计算单位不能和批次库存单位一致!'));
      }else{
        callback();
      }
    };
    ...
    return{
      ...
      remark:'',
      rules:{
        transitionNums:[{required: true, message: '请填写转换数量', trigger: 'blur' },{ validator:isExceed, trigger: 'blur'}],
        transitionPiece:[{required: true, message: '请填写转换件数', trigger: 'blur'}],
        subUnit:[{required: true, message: '请填写计算单位', trigger: 'blur'},{ validator: isFit, trigger: 'blur' }],
  ...
  components:{
    subunitDialog: () => import("./addCommodity/dialogmod/subunitDialog.vue"), // 计算单位
  },
  methods:{
    
      subunitChange(val) {
        this.editGoodsMsg.subUnit = val.subunit; // 计算单位
        this.editGoodsMsg.subUnit2 = val.subUnit2; // 二级计算单位，2021-11-15需求优化新增
        this.editGoodsMsg.subUnit2Weight = val.subUnit2Weight || 0; // 二级计算单位重量，2021-11-15需求优化新增
        this.editGoodsMsg.subUnit3 = val.subUnit3; // 三级计算单位，2021-11-15需求优化新增
        this.editGoodsMsg.subUnit3Weight = val.subUnit3Weight || 0; // 三级计算单位，2021-11-15需求优化新增

        let paramS = this.editGoodsMsg.subUnit
        paramS += this.editGoodsMsg.subUnit2 != '' ? '(' : ''
        paramS += this.editGoodsMsg.subUnit3Weight == 0 ? '' : this.editGoodsMsg.subUnit3Weight + this.editGoodsMsg.subUnit3
        paramS += this.editGoodsMsg.subUnit3 != '' ? '*' : ''
        paramS += this.editGoodsMsg.subUnit2Weight == 0 ? '' : this.editGoodsMsg.subUnit2Weight + this.editGoodsMsg.subUnit2
        paramS += this.editGoodsMsg.subUnit2 != '' ? '/' + this.editGoodsMsg.subUnit : ''
        paramS += this.editGoodsMsg.subUnit2 != '' ? ')' : ''
        this.editGoodsMsg.subUnitFocus = paramS
        console.log(this.editGoodsMsg.subUnitFocus)
      },
    subunitFocus() {

          this.handleEdit("subunitDialog", "计算单位", {productid:this.goodMsg.categoryThird});
    ...
    guobangSubmit(type){
      ...
      if(type == '批次'){
        this.form.transitionNums = sum1;
        this.form.transitionPiece = sum2;
      }else{
    ...
    submit(type){
      ...
      for(let i=0;i<this.tableData.length;i++){
        if(!this.tableData[i].transitionNums){
          // console.log(i+1,'没有编辑')
          this.$error('编号'+ (i+1) + '的批次没有被编辑，请移除或编辑后再做提交！');
          issubmit = false;
        }
      }
      if(this.rukuchengbenzonge == 0){
        this.$error('请编辑新商品信息！')
        issubmit = false
      }
      if(issubmit){
        console.log(this.goodMsg)
        let str = this.goodMsg.name
        let str2 = '|'
        let name = str.split(str2)[0]
        let res = {
          productid:this.goodMsg.productId,
          productName:name,
          variety:this.goodMsg.variety,
          remark:this.remark,
          regionName:this.goodMsg.regionName,
          addInfo:[],
          allBasketinfo:[],
          isPell:this.editGoodsMsg.isQp == '1' ? 0 : 1,
          allbasketnums:this.getsumown ? this.getsumown : '0',
          conUnit:this.editGoodsMsg.subUnit,
          subUnit2:this.editGoodsMsg.subUnit2,
          subUnit2Weight:this.editGoodsMsg.subUnit2Weight,
          subUnit3:this.editGoodsMsg.subUnit3,
          subUnit3Weight:this.editGoodsMsg.subUnit3Weight,
          targetNums:this.rukushuliang,
          pieceNums:this.editGoodsMsg.nums2,
          pieceUnit:this.editGoodsMsg.pieceUnit,
          costPrice:this.rukuchengbendanjia,
          addCost:this.editGoodsMsg.chengben,
          costTotalPrice:this.rukuchengbenzonge
        }
        for(let i=0;i<this.submitBasketMsg.length;i++){
          for(let j=0;j<this.submitBasketMsg[i].modellist.length;j++){
            res.allBasketinfo.push({
              basketName:this.submitBasketMsg[i].basektName,
              basketId:this.submitBasketMsg[i].basketId,
              color:this.submitBasketMsg[i].modellist[j].color,
              unit:0,
              basketNum:this.submitBasketMsg[i].modellist[j].nums,
              weight:this.submitBasketMsg[i].modellist[j].weight
            })
          }
        }
        ...
        for(let i=0;i<this.tableData.length;i++){
          res.addInfo.push({
            ...
            conversionNum:this.tableData[i].transitionNums,
            conpieceNum:this.tableData[i].transitionPiece,
            allvacateNum:this.tableData[i].sumSoar,
            costTotalPrice:this.tableData[i].transitionCost,
            vacatebasket:[]
          })
          for(let j=0;j<this.tableData[i].batchGetBasket.length;j++){
            for(let k=0;k<this.tableData[i].batchGetBasket[j].info.length;k++){
              res.addInfo[i].vacatebasket.push({
                basketId:this.tableData[i].batchGetBasket[j].basketId,
                basketName:this.tableData[i].batchGetBasket[j].basketName,
                basketColor:this.tableData[i].batchGetBasket[j].info[k].color,
                vacateNum:this.tableData[i].batchGetBasket[j].info[k].soar
            })
            }
          }
        }
        getunitConversion(res).then(resp=>{
          console.log(resp)
          if(resp.status == 200){
            this.$success('转换成功！')
            if(type == 1){
        ...
        if(valid){
          for(let i=0;i<this.tableData.length;i++){
            if(this.tableData[i].batchid == this.editBatchMsg.batchid){
              this.$set(this.tableData[i],'transitionNums',this.form.transitionNums),
              this.tableData[i].transitionPiece = this.form.transitionPiece,
              this.tableData[i].batchGetBasket = this.form.batchGetBasket,
              this.tableData[i].sumSoar = this.sumSoar,
              this.tableData[i].transitionCost = this.getCost
            }
          }
    ...
    tableDataEdit(row,index){
      ...
      if(row.batchGetBasket){
        this.form.transitionNums = row.transitionNums
        this.form.transitionPiece = row.transitionPiece
        this.form.batchGetBasket = row.batchGetBasket
        ...
        getBatch(this.dialogqueryData).then(resp=>{
          ...
          for(let i=0;i<res.length;i++){
            if(res[i].batchid == this.$route.query.id2){
              ...
              console.log(this.$route.query.id2,res)
            }
          }
        })
      }
  ...
  computed:{
    ...
    getZongliang(){//sumSoar
      ...
      for(let i=0;i<this.tableData.length;i++){
        if(this.tableData[i].transitionNums){
          res += this.tableData[i].transitionNums
        }
      }
      return res;
    },
    getChengben(){
      let res = 0
      for(let i=0;i<this.tableData.length;i++){
        if(this.tableData[i].transitionCost){
          res += this.tableData[i].transitionCost
        }
      }
      return res;
    },
    ...
    getCost(){
      if(this.editBatchMsg.remainCount >= this.form.transitionNums){
        return this.form.transitionNums * this.editBatchMsg.costPrice
      }else{
