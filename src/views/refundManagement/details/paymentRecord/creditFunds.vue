          ...
          >
            ...
            >
              <template slot-scope="scope">
                ...
                <span
                  v-else-if="item.prop == 'timestamp'"
                >
                  {{ scope.row.timestamp | parseTime }}
                </span>
                <span
                  v-else-if="item.prop == 'comeInPrice'"
                >
            ...
            <u-table-column label="操作" :width="80" align="left">
              <template slot-scope="scope">
                <el-button type="text" style="margin-right: 15px" @click="handleShow(scope)">
                  查看</el-button
                >
              </template>
            </u-table-column>
...
export default {
  ...
  methods: {
    ...
    handleShow(val) {
      ...
      // 销售单，退货单，退款单，售出单，退筐单，记账未收(筐子+商品)，报废单
      // 采购单，退货单，退款单，购入单，退筐单，记账未付(筐子+商品)，自购单

      if (type == "OrderBasket") {
        // OrderBasket(筐子订单) 筐子售出单
        this.$router.push({
          path: "/frameManage/basketSold",
          query: {
            orderId: val.row.uid,
