<template>
  <Page class="page_style">
    ...
    <div class="aboutOrder">
      <el-row>
        <el-col :span="8">关联销售单:<el-button type="text" style="color:#0BA198;">{{pageData.orderSellNo}}<i class="el-icon-arrow-right"></i></el-button></el-col>
        <el-col :span="8">销售时间:<span>{{ toDate(pageData.orderSellTime) }}</span></el-col>
      </el-row>
    </div>
    <div class="button_layout">
      <div style="width:50%;padding-left:20px;"><el-button type="text" v-if="showOrderMsg.isShowCR" @click="paymentRecord()">付款记录</el-button></div>
    ...
    <el-dialog
      ...
      <div style="height:600px;overflow-y: auto;">
        ...
        <div v-if="isShowTable.table_area_1">
          <!-- ##########################################################################################################################  退货筐 -->
          <el-table :data="basketResults.return_basket" style="margin-bottom: 10px; border: 1px solid #e9e9e9" v-if="isShowTable.return">
            <el-table-column width="170" label="退入筐子" :formatter='frommartter' prop="userbasketname" :render-header="renderHeader"></el-table-column>
            <el-table-column  label="颜色" prop="userbasketcolor"></el-table-column>
            <el-table-column  label="退入筐子" prop="outreturnbasketcount" :formatter='frommartter'></el-table-column>
          ...
          <div class="total_style" style="margin-bottom:15px;">
            ...
            <div class="total_styl_layout" style="float:right;">退入筐子:{{dialogBasketMsg.returnBasketCount+'个'}}</div>
          </div>
        </div>
        <div v-if="isShowTable.table_area_2">
          ...
          <div class="total_style">
            <div class="total_styl_layout" style="font-weight: 900;color: rgba(1,6,33,0.9);">小计：</div>
            <div class="total_styl_layout" style="float:right;">使用自家筐子:{{dialogBasketMsg.selfBasketCount+'个'}}</div>
        ...
        <div>
          <div class="total_styl_layout" style="font-weight: 900;color: rgba(1,6,33,0.9);">
            共退回筐子{{ dialogBasketMsg.returnBasketCount }}个,使用自家筐子{{ dialogBasketMsg.selfBasketCount }}个
          </div>
...
export default {
  ...
  methods:{
    ...
    isShowArea(){
      if(this.isShowTable.carry == false && this.isShowTable.weigh == false && this.isShowTable.piece == false && this.isShowTable.self == false && this.isShowTable.return == false){
    ...
    showBasketTbale(){
      ...
      this.isShowTable.return = this.basketResults.return_basket.length > 0 ? true : false;
      this.isShowArea();
    },
    renderHeader2(){
      return (
    ...
    statusIs1(){//待确认
      ...
      if(this.pageData.isReturnProduct == 0){
        button.splice(1,1)
      }
      this.showOrderMsg.msg = res;
      this.showOrderMsg.button = button;
    ...
    getOrderMsg(){
      ...
      // let type = 5
      // this.pageData.isReturnProduct = 0;
