    ...
    <div class="cardBox" style="background-color: rgba(0, 0, 0, 0.04);padding:24px;">
      ...
      <div class="card-box">
        <el-tooltip placement="bottom-start">
          <div slot="content" style="padding:8px 30px 8px 5px;">
            <div>
              <span style="margin-right:12px">未收销售款</span>
              <span>{{ priceSum.unReceive }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">已收货款</span>
              <span>{{ priceSum.receivedAmount }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">未付采购款</span>
              <span>{{ priceSum.unPay }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">已付货款</span>
              <span>{{ priceSum.payAmount }}元</span>
            </div>
          </div>
          <el-button style="border:none;background-color: #f5f5f5;padding:0px;text-align:left;">
            <span class="card-title">往来款余额</span>
      ...
      <div class="card-box">
        <el-tooltip placement="bottom-start">
          ...
          <el-button style="border:none;background-color: #f5f5f5;padding:0px;text-align:left;">
            ...
            <div class="card-price" style="margin-top:10px">
              {{ moneyFormat(priceSum.businessBalance) }}
            </div>
          </el-button>
        </el-tooltip>
      </div>
      ...
      <div class="card-box">
        <el-tooltip placement="bottom-start">
          <div slot="content" style="padding:8px 30px 8px 5px;">
            <div>
              <span style="margin-right:12px">预付款</span>
              <span>{{ canDeduction.advanceAmount }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">借款</span>
              <span>{{ canDeduction.borrowAmount }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">其他</span>
              <span>{{ canDeduction.otherAmount }}元</span>
            </div>
          </div>
          <el-button style="border:none;background-color: #f5f5f5;padding:0px;text-align:left;">
            <span class="card-title">我转出的</span>
            <div class="card-price" style="margin-top:10px">
              {{ moneyFormat(priceSum.canDeductionAmount) }}
            </div>
          </el-button>
        </el-tooltip>
      </div>
      ...
      <div class="card-box">
        <el-tooltip placement="bottom-start">
          <div slot="content" style="padding:8px 30px 8px 5px;">
            <div>
              <span style="margin-right:12px">预付款</span>
              <span>{{ shouldReturn.advanceAmount }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">借款</span>
              <span>{{ shouldReturn.borrowAmount }}元</span>
            </div>
            <div style="margin-top:16px">
              <span style="margin-right:12px">其他</span>
              <span>{{ shouldReturn.otherAmount }}元</span>
            </div>
          </div>
          <el-button style="border:none;background-color: #f5f5f5;padding:0px;text-align:left;">
            <span class="card-title">我收到的</span>
            <div class="card-price" style="margin-top:10px">
              {{  moneyFormat(priceSum.shouldReturnAmount) }}
            </div>
          </el-button>
        </el-tooltip>
      </div>
    ...
    <el-form inline ref="search-form" style="width: 100%;margin-top:18px;">
      ...
      <el-form-item class="custom-form-item">
        <div class="custom-date-picker">
          <span>创建日期</span>
          <el-date-picker size="small" type="daterange" v-model="queryData.timestampList" start-placeholder="开始日期"
            end-placeholder="结束日期" value-format="timestamp" prefix-icon></el-date-picker>
  ...
  export default {
    ...
    data() {
      return {
        ...
        priceSum: {
          balance: 0, //往来款余额
          businessBalance: 0, // 经营往来款余额
          canDeductionAmount: 0, // 我转出的
          shouldReturnAmount: 0, // 我收到的
          unPay: 0, // 未付采购款
          unReceive: 0, // 未收销售款
          payAmount: 0, // 已付货款
    ...
    methods: {
      getList(arg) {
        ...
        this.tableData = [];
        businessPaymentsDetail(param)
          .then(res => {
            this.tableData = res.data[0].modelList;
            this.pagination.total = res.data[0].allCount; //数据总条数
            this.loading = false;
            this.priceSum = {
              balance: res.data[0].balance, //往来款余额
              businessBalance: res.data[0].businessBalance, // 经营往来款余额
              canDeductionAmount: res.data[0].canDeductionAmount, // 我转出的
              shouldReturnAmount: res.data[0].shouldReturnAmount, // 我收到的
            ...
            // 我转出的;
            this.canDeduction = {
              advanceAmount: res.data[0].canDeduction.advanceAmount, // 预付款
              borrowAmount: res.data[0].canDeduction.borrowAmount, // 借款
              otherAmount: res.data[0].canDeduction.otherAmount // 其他
            };
            // 我收到的;
            this.shouldReturn = {
              advanceAmount: res.data[0].shouldReturn.advanceAmount, // 预付款
              borrowAmount: res.data[0].shouldReturn.borrowAmount, // 借款
              otherAmount: res.data[0].shouldReturn.otherAmount // 其他
            };
          });
      },
      // 数值取两位小数
