<template>
  <!-- 收到的需求单 -->
  <div
    class="requestsheetReceivedModel"
    :style="{ '--screenHeight': $store.getters.screenHeight - 390 + 'px' }"
  >
    <div class="adv-header-title">收到的需求单</div>
    <div class="flex_dom">
        ...
        <div class="paging">
          ...
          <div>
            <span
              ...
              种商品</span
            >
            <el-button type="primary" size="small">开销售单</el-button>
            <el-button type="primary" size="small" @click="handleShow"
              >向上开需求单</el-button
            >
          </div>
        </div>
    ...
    <upDemandOrder ref="upDemandOrder"></upDemandOrder>
  </div>
</template>
...
import upDemandOrder from "../../dialogmod/upDemandOrder.vue";

export default {
  name: "requestsheetReceived",
  props: {
    toUserId: {
      type: String,
    },
  ...
  methods: {
    ...
    handleDelete(val) {
      this.$confirm("确认删除吗?", "提示", {
        ...
        .then(() => {
          let param = {
            DraftId: val.uid, // 草稿箱主键id 必填
            typeView: 2, // 1 需求单草稿箱 2 销售单草稿箱  必填
          };
    ...
    handleShow() {
      this.$refs.upDemandOrder.edit({ custmorId: this.toUserId });
      this.$refs.upDemandOrder.title = "选择客户  向上开需求单";
      this.$refs.upDemandOrder.disableSubmit = false;
      this.$refs.upDemandOrder.isVisiable = true;
    },
    // 分页
