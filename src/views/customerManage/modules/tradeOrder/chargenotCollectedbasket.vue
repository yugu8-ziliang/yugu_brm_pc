    ...
    <div class="cardBox" style="background-color: rgba(0, 0, 0, 0.04);padding:24px;">
      ...
      <div class="card-box">
        ...
        <div class="card-price">
          {{ moneyFormat(priceSum.transferPrice) }}
        </div>
      </div>
    </div>
    <!-- 搜索框 -->
      ...
      <u-table ref="plTable" class="utable" :height="$store.getters.screenHeight - 440 + 'px'" :border="false"
        ...
        <u-table-column v-for="(item, index) in columns" :key="index" :resizable="item.resizable"
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="item.prop=='transferNoteTimestamp'">
              <span v-if="scope.row[item.prop] == 0" style="color: #e02020">未成交</span>
              <span v-else>{{scope.row[item.prop]|parseTime}}</span>
            </span>
            <span v-else-if="item.prop == 'status'">
              {{
              scope.row[item.prop] | isOrderStatus3(scope.row[item.prop])
              }}
            </span>
            <span v-else-if="item.prop=='transferPrice'">
              {{
...
<script>
  ...
  const columns = [{
    ...
    {
      prop: "transferNoteTimestamp",
      label: "成交时间",
      align: "left"
    }
  ];
  export default {
    name: "chargenotCollectedbasket",
    data() {
      return {
        columns: columns,
        priceSum: {
        ...
        queryData: {
          status: "0", //0 全部 1 待对方确认 2 待确认 3 已成交
          timestampList: [], //创建日期
          confirmTimestampList: [], //成交日期
          typeView: 3 //1 记账未收货款 2 记账未付货款 3 记账未收筐子款 4 记账未付筐子款
        },
        typeViews: Object.freeze({
          bookedGoodsunpaid: 1,
          accountUnpaidgoods: 2,
          chargenotCollectedbasket: 3,
          chargeUnpaidbasket: 4
        }),
        pagination: {
          page: 1, // 页码
          limit: 10, // 分页大小
    ...
    methods: {
      getList(arg) {
        ...
        getTransFerNote(param)
          .then(res => {
            if (res.status == 200) {
              this.tableData = res.data[0].modelList;
              this.priceSum = {
                transferPrice: res.data[0].transferPrice //总金额
              };
