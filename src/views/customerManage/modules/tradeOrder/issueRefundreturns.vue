<template>
  ...
  <el-scrollbar style="height: 100%">
    <div class="cardBox" style="background-color: rgba(0, 0, 0, 0.04);padding:24px;">
      ...
      <div class="lineH"></div>
      <div class="card-box">
        <span class="card-title">商品退款金额</span>
        <div class="card-price">
      ...
      <div class="lineH"></div>
      <div class="card-box">
        <span class="card-title">载货筐子退款金额</span>
        <div class="card-price">
      ...
      <u-table ref="plTable" class="utable" :height="$store.getters.screenHeight - 440 + 'px'" :border="false"
        ...
        <u-table-column v-for="(item, index) in columns" :key="index" :resizable="item.resizable"
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="item.prop=='orderTotalPrice'|| item.prop=='actualPayPrice'|| item.prop=='receivePrice'">
              {{
...
<script>
  ...
  export default {
    name: "issueRefundreturns",
    data() {
      return {
        columns: columns,
        priceSum: {
          orderTotalPrice: 10.0, //总金额
          productTotalPrice: 10.0, // 商品退款金额
          basketTotalPrice: 0 // 载货筐子退款金额
        },
        queryData: {
          status: "0",
          timestampList: [], //创建日期
          confirmTimestampList: [], //成交日期
          typeView: 1 //1 发出的退货退款 2 收到的退货退款
        },
        typeViews: Object.freeze({
          issueRefundreturns: 1,
          receivedReturnsrefunded: 2
        }),
        pagination: {
          page: 1, // 页码
          limit: 10, // 分页大小
    ...
    methods: {
      getList(arg) {
        ...
        getOrderReturn(param)
          .then(res => {
            if (res.status == 200) {
              this.tableData = res.data[0].modelList;
              this.pagination.total = res.data[0].modelListCount; //分页前数据条数
              this.priceSum = {
                orderTotalPrice: res.data[0].orderTotalPrice, //总金额
                productTotalPrice: res.data[0].productTotalPrice, // 商品退款金额
                basketTotalPrice: res.data[0].basketTotalPrice // 载货筐子退款金额
              };
            }
          });
      },
