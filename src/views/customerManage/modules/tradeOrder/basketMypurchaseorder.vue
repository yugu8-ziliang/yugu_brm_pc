        ...
        <u-table-column v-for="(item, index) in columns" :key="index" :resizable="item.resizable"
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="item.prop == 'status'">
              {{
              scope.row[item.prop] | isOrderStatus6(scope.row[item.prop])
              }}
            </span>
            <span v-else-if="item.prop=='basketTotalPrice'|| item.prop=='actualPayPrice'|| item.prop=='receivePrice'">
              {{
