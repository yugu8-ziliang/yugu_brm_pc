    ...
    <el-form inline ref="search-form" style="width: 100%;margin-top:18px;">
      <el-form-item>
        <el-select v-model="queryData.status" size="small">
          ...
          <el-option label="待对方确认" value="3"></el-option>
          <el-option label="待收款" value="4"></el-option>
        ...
        <u-table-column v-for="(item, index) in columns" :key="index" :resizable="item.resizable"
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="item.prop == 'status'">
              {{
              scope.row[item.prop] | isOrderStatus5(scope.row[item.prop])
              }}
            </span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        queryData: {
          ...
          typeView: 2 //1 筐子购入单 2 筐子售出单
        },
        typeViews: Object.freeze({
          basketPurchaseorder: 1,
          basketSaleorder: 2
        }),
        pagination: {
          page: 1, // 页码
    ...
    methods: {
      getList(arg) {
        ...
        getOrderBasket(param)
          .then(res => {
            if (res.status == 200) {
              this.tableData = res.data[0].modelList;
              this.orderTotalPrice = res.data[0].orderTotalPrice; //头部总金额
