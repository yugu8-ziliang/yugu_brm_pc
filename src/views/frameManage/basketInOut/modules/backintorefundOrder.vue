  ...
  <PageNoScroll class="orderDetailsModel">
    <div>
      ...
      >
        <div>
          <el-descriptions title="订单信息">
            ...
            <!-- <el-descriptions-item
              v-if="tableData.status==4||tableData.status==5||(tableData.orderType==1&&tableData.status==3)"
              label="退筐时间">{{tableData.confirmTimestamp| parseTime}}</el-descriptions-item> -->
          </el-descriptions>
          ...
          >
            <el-button
              ...
              >退回修改</el-button
            >
            <el-button size="small" type="primary">确认退款</el-button>
          </div>
          <!-- 平台待对方收款 -->
          <div
            class="type-button"
          ...
          >
            <div>
              <span class="type-button-left"></span>
            </div>
            <div>
              <el-button
                class="type1-button-right"
              ...
              <el-button size="small" type="primary">添加退款</el-button>
            </div>
          </div>
          <!-- 非平台未结清 -->
          <div
          ...
          >
            ...
            <div>
              <el-button size="small" type="primary">继续退款</el-button>
            </div>
          </div>
        </div>
      ...
      <el-dialog :title="modalTitle" :visible.sync="modalShow" width="30%">
        <!-- 取消付款 -->
        <div v-if="btnType === 'cancelPayment'">确认取消付款？</div>
        <!-- 修改订单 -->
        <div v-else-if="btnType === 'modifyOrder'">确认修改订单？</div>
        <!-- 取消订单 -->
        <div v-else-if="btnType === 'cancelOrder'">确认取消订单？</div>
        <!-- 撤回修改 -->
  ...
  methods: {
    ...
    makeSure() {
      ...
      if (this.btnType === "modifyOrder") {
        this.$router.push({
          path: "/refundManagement/changebackBasket",
          query: {
            orderBasketReturnId: this.orderBasketReturnId,
          },
        });
