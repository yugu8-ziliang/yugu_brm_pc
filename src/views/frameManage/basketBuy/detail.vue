<template>
  <Page>
    ...
    <el-descriptions title="" :column="3">
      ...
      <el-descriptions-item label="购入时间">{{ toDate(orderMsg.buyTimestamp) }}</el-descriptions-item>
      <el-descriptions-item label="订单金额">{{ currencyFormat(orderMsg.totalPrice) }}</el-descriptions-item>
      <el-descriptions-item label="已付金额" v-if="type == 0">
        {{ currencyFormat(orderMsg.actualPayPrice) }}
      </el-descriptions-item>
      <el-descriptions-item label="剩余未付" v-if="type == 0">
        {{ currencyFormat(orderMsg.shouldPayPrice) }}
      </el-descriptions-item>
      <el-descriptions-item label="待确认金额" v-if="type != 0">
      ...
      <el-descriptions-item label="成交时间" v-if="type == 0">
        {{ toDate(orderMsg.confirmTimestamp) }}
      </el-descriptions-item>
    </el-descriptions>
    <div style="text-align:right;">
      <el-button style="border:1px solid #0BA198;color:#0BA198;">退筐退款</el-button>
      <el-button style="border:1px solid #0BA198;background-color: #0BA198;color:#fff;">添加付款</el-button>
    </div>
...
import {getInfo} from '@/request/api/basketBuy/index.js'
export default{
  data(){
    return{
